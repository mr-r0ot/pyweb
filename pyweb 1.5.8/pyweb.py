...
def page(folder="templates",page="index.html", title="Pyweb Site",background=False,background_linear_gradient=False,font_page="sans-serif",charset="UTF-8",lang="en",content="IE=edge",page_center=False,install_python=False):
    try:
...
class PY:
    ...
    def config_python(code,page="index.html"):
        __html=open(page,"a+")
        code=(f'''  <py-config>{code}</py-config> ''')
        __html.write(code)
        __html.close()

    def run_python(code,page="index.html"):
        __html=open(page,"a+")
        code=(f'''  <py-script>{code}</py-script> ''')
        __html.write(code)
        __html.close()
